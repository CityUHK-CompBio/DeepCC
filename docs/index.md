...
A functional spectrum records how strongly each gene set is enriched
among one sample’s most highly expressed genes. With the default
`scale = TRUE`, each gene is centred across the samples you supply, each
sample is ranked on those centred values, and the weighted running-sum
enrichment statistic is computed per sample.

This is not a log fold change and no group labels are involved. A score
is relative to the cohort in `eps`, so the same sample receives
different scores in a different cohort; keep the cohort fixed between
training and prediction. A single row with `scale = TRUE` centres to
zero, so use
[`getFunctionalSpectrum()`](https://cityuhk-compbio.github.io/DeepCC/reference/getFunctionalSpectrum.md)
for genuine single-sample scoring.
...
| Newest MSigDB release | varies | [`get_msigdbr()`](https://cityuhk-compbio.github.io/DeepCC/reference/get_msigdbr.md) |
...
release is adopted, so a given DeepCC version always resolves to the
same gene sets.
...
Those models were saved by Keras 2.
[`load_DeepCC_model()`](https://cityuhk-compbio.github.io/DeepCC/reference/load_DeepCC_model.md)
rebuilds them from their recorded architecture, so they load under the
current keras3 without conversion. Saving a loaded model writes it in
the current format.
